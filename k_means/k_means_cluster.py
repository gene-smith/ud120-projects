...
exercised_stock = []
for k in data_dict:
    if data_dict[k]["salary"]:
        if data_dict[k]["salary"] == 'NaN':
            continue
        salary.append(float(data_dict[k]["salary"]))
    if data_dict[k]["exercised_stock_options"]:
        if data_dict[k]["exercised_stock_options"] == 'NaN':
            continue
        exercised_stock.append(float(data_dict[k]["exercised_stock_options"]))

salary = [min(salary), 200000., max(salary)]
exercised_stock = [min(exercised_stock), 1000000., max(exercised_stock)]

salary = numpy.array([[e] for e in salary])
exercised_stock = numpy.array([[e] for e in exercised_stock])

scaler_salary = MinMaxScaler()
scaler_stock = MinMaxScaler()

rescaled_salary = scaler_salary.fit_transform(salary)
rescaled_stock = scaler_stock.fit_transform(exercised_stock)
